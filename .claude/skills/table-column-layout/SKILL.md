...
## Pending code — Created By / User cell (2026-07-05)

Spec + skill updated for the optional **Created By (User)** column; **code is not yet shipped** (that round was spec/guideline only). To implement:

1. **New `user-cell` Cell type** (`packages/components/src/Cell/Cell.tsx`) — avatar (photo via `avatarSrc`, else empty-teal initials from `name`) + name + `email` (new prop). Add to `CellType`, `COL_TYPES`, and the render switch; add a Storybook story (photo variant + empty-teal-initials variant). Read the exact Figma "User cell" structure via the bridge first.
2. **Created By column** — the playground's `CREATED_BY_COLUMN` is currently a fixed `{ role: 'utility', width: 140 }` rendering a plain `sample` text name. Change it to a **low-weight Primary**: `{ label: 'Created By', role: 'primary', flex: 0.5, minWidth: 200 }`, rendering the `user-cell`. Offer it on all Order-family instances via the Columns control (splice between Last Updated and Status).
3. **Mock data** — clustered/randomized users (name + email + optional `avatarSrc`) so some rows show a photo and others the empty-teal initials.
4. Rebuild the `@leta/components` dist; verify in Storybook + the playground; then commit.
5. **Separate (not this spec):** swap the Unassigned **Basic Filter** dropdown → **Filter Group** (Recipient name + Created By options). Tracked as a later code task, outside this column spec.
